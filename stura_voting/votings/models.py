...
class Period(models.Model):
    name = models.CharField(
        max_length=150,
        help_text='Name der Abstimmungsperiode, z.B. "Sommersemester 2018"',
        unique=True,
        default=get_semester_name)
    created = models.DateTimeField(
        help_text='Erstellungszeitpunkt',
        default=timezone.now)
    start = models.DateField(
        blank=True,
        null=True,
        help_text='Start der Periode',
        default=get_semester_start)
    end = models.DateField(
        blank=True,
        null=True,
        help_text='Ende der Periode',
...
class VotersRevision(models.Model):
    period = models.ForeignKey(
        ...
        help_text='Periode für diese Revision')
    created = models.DateTimeField(
        help_text='Erstellungszeitpunkt',
        default=timezone.now)
...
class Voter(models.Model):
    revision = models.ForeignKey(
        'VotersRevision',
        on_delete=models.CASCADE,
        help_text='Revision this user is used in')
    name = models.CharField(max_length=150,
                            help_text='Name of the voter / group')
    weight = models.PositiveIntegerField(
        help_text='Weight of the votes, i.e. how many votes the voter actually casts')
...
class VotingCollection(models.Model):
    ...
    revision = models.ForeignKey(
        'VotersRevision',
        on_delete=models.CASCADE,
        help_text='Group of voters for this voting')
...
class VotingGroup(models.Model):
    name = models.CharField(
        max_length=150,
        help_text='Name of the group, for example "Financial Votes"')
    collection = models.ForeignKey(
        'VotingCollection',
        on_delete=models.CASCADE,
        help_text='Collection this group belongs to')
...
class MedianVoting(models.Model):
    ...
    value = models.PositiveIntegerField(
        help_text='Value for this voting (for example 2000 (cent). Always meassured in cents, pence etc.)')
    majority = models.CharField(
        max_length=10,
        help_text='Required majority for voting',
        choices=MAJORiTY_CHOICES,
        default=FIFTY_MAJORITY)
    absolute_majority = models.BooleanField(
        help_text='Set to true if all voters should be considerd, even those who did not cast a vote. Thes voters will be treated as if they voted for 0€',
        default=False)
    currency = models.CharField(
        max_length=10,
        blank=True,
        help_text='Currency of the vote, for example "$" or "€". For example value=100 and currency=€ means 1,00€.',
        default='€')
    group = models.ForeignKey(
        'VotingGroup',
        on_delete=models.CASCADE,
        help_text='Group this voting belongs to')
    voting_num = models.PositiveIntegerField(
        help_text='Abstimmungsnummer innerhalb der Gruppe')
...
class SchulzeVoting(models.Model):
    ...
    majority = models.CharField(
        ...
        default=FIFTY_MAJORITY)
    absolute_majority = models.BooleanField(
        help_text='Set to true if all voters should be considerd, even those who did not cast a vote. Thes voters will be treated as if they voted for no (which is considered to be the last option)',
        default=False)
    group = models.ForeignKey(
        'VotingGroup',
        on_delete=models.CASCADE,
...
class SchulzeOption(models.Model):
    ...
    option_num = models.PositiveIntegerField(
        help_text='Optionsnummer innerhalb der Schulze Abstimmung')
...
class MedianVote(models.Model):
    ...
    voting = models.ForeignKey(
        'MedianVoting',
        on_delete=models.CASCADE,
        help_text='The voting in question')
